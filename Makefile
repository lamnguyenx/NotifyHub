...
mock:
	src/notifyhub/frontend/__tests__/send_mock_notis.py

chrome:
	@if [ "$$(uname)" = "Darwin" ]; then \
		open -a "Google Chrome" --args --remote-debugging-port=9222 --user-data-dir=/tmp/chrome-dev-profile; \
	else \
		setsid nohup google-chrome --remote-debugging-port=9222 --user-data-dir=/tmp/chrome-dev-profile >/dev/null 2>&1 & \
	fi
