...
class TestCLIArgumentParsing:
    ...
    def test_custom_port(self):
        with patch(
            "sys.argv",
            ["notifyhub-push", "--port", "9999", '{"message": "test message"}'],
        ):
            with patch("notifyhub.cli.requests.post") as mock_post:
                mock_response = MagicMock()
                mock_response.json.return_value = {"success": True}
...
class TestCLIErrorHandling:
        ...
        with patch("sys.argv", ["notifyhub-push", '{"message": "test"}']):
            with patch("notifyhub.cli.requests.post") as mock_post:
                mock_post.side_effect = (
                    notifyhub.cli.requests.exceptions.RequestException(
                        "Connection failed"
                    )
                )
    ...
    def test_timeout_error(self, capsys):
        with patch("sys.argv", ["notifyhub-push", '{"message": "test"}']):
            with patch("notifyhub.cli.requests.post") as mock_post:
                mock_post.side_effect = notifyhub.cli.requests.exceptions.Timeout(
                    "Timeout"
                )
