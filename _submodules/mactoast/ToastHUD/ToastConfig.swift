...
struct ToastConfig {
    ...
    var iconText: String?  // Text to render as a circular avatar icon
    var iconBg: String?  // Hex background color for the avatar circle
    ...
    static func fromCommandLine() -> ToastConfig {
        ...
        while i < args.count {
            ...
            if arg == "--width", let value = readValue(after: &i), let w = Double(value) {
                ...
                config.icon = value
            } else if arg == "--icon-text", let value = readValue(after: &i) {
                config.iconText = value
            } else if arg == "--icon-bg", let value = readValue(after: &i) {
                config.iconBg = value
